			...
			<form id="subs{$r.sd_id}" name="subs{$r.sd_id}">
				<div class="col-md-4 col-sm-12 col-xs-12 subs-box" >
					<div class="col-md-12 subs-upper subs-upper-resp">
						...
						<h3 align="center" {if $r.active== "YES"} style="margin-bottom: -20px;"{/if}>{$r.name}{if $r.active== "YES"} <b>(Active)</b>{/if}</h3>
						{if $r.active== "YES"}
			...
			if (radioCheck == false) {
				...
				if (formValue == "" || formValue == null) {
					...
					if (data[0] > 0) {
						...
						window.location.href = "employer.php?action=insert_subscription&sr_id=" + sr_id + "&uid=" + uid;
					}
				}
			}
		}
